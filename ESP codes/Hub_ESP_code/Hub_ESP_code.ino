...
const char* ssid = "Orange B";          // Wi-Fi Name
const char* password = "Basemelgamal2";  // Wi-Fi Password
...
void OnDataRecv(const esp_now_recv_info *info, const uint8_t *incomingDataPtr, int len) {
  if (len != sizeof(struct_message)) {
    Serial.printf("[ESP-NOW] Ignored packet with invalid size: %d\n", len);
    return;
  }
  ...
  if (packet.id[0] == '\0') {
    Serial.println("[ESP-NOW] Ignored packet with empty node ID");
    return;
  }
  ...
  // Preserve a node's slot when packet arrival order changes.
  for (int i = 0; i < 4; i++) {
    if (nodes[i].active && strcmp(nodes[i].id, packet.id) == 0) {
      targetIndex = i;
      break;
    }
  }

  // A new node may use an empty slot or reclaim one that has been stale for 30 s.
  if (targetIndex == -1) {
    for (int i = 0; i < 4; i++) {
      if (!nodes[i].active || (now - nodes[i].lastSeen >= 30000)) {
        targetIndex = i;
        break;
      }
    }
  ...
  if (targetIndex == -1) {
    Serial.printf("[ESP-NOW] No free node slot for '%s'\n", packet.id);
    return;
  }

  strlcpy(nodes[targetIndex].id, packet.id, sizeof(nodes[targetIndex].id));
  nodes[targetIndex].v = packet.voltage;
  nodes[targetIndex].a = packet.current;
  nodes[targetIndex].w = packet.power;
  nodes[targetIndex].overloaded = packet.isOverloaded;
  nodes[targetIndex].lastSeen = now;
...
void handleRelayCommand() {
  if (server.hasArg("id") && server.hasArg("state")) {
    ...
    if (!knownNode) {
      server.send(404, "application/json", "{\"status\":\"error\",\"message\":\"node offline or unknown\"}");
      return;
    }
    ...
    esp_err_t sendResult = esp_now_send(broadcastAddress, (uint8_t *) &cmd, sizeof(cmd));

    if (sendResult != ESP_OK) {
      Serial.printf("[HTTP] Failed to queue relay command for %s (error %d)\n", cmd.id, sendResult);
      server.send(503, "application/json", "{\"status\":\"error\",\"message\":\"ESP-NOW send failed\"}");
      return;
    }
